...
export class MinimaxStrategy extends Strategy {
    ...
    async chooseMove(gameState) {
        ...
        for (const move of moves) {
            ...
            const score = this.minimax(simulatedBoard, this.depth - 1, false, color);
    ...
    minimax(board, depth, isMaximizing, maximizingColor) {
        if (depth === 0) {
        ...
        if (isMaximizing) {
            ...
            for (const move of moves) {
                ...
                const ev = this.minimax(simulatedBoard, depth - 1, false, maximizingColor);
            ...
            for (const move of moves) {
                const simulatedBoard = this.simulateMove(board, move);
                const ev = this.minimax(simulatedBoard, depth - 1, true, maximizingColor);
